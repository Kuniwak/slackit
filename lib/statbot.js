 ...
 *
 * Necessary:
 *  - `options.teamname`: Name of the your team (equals the sub domain of your slack page)
 ...
 *  - `options.channel` (default `#general`): Default channel to send a message by the bot.
...
Statbot.prototype.getServerMechanism = function() {
  ...
  if (!statbot.promisedServerGetter_) {
    ...
    expressServer.post(outgoingHookPath, function(req, res) {
      statbot.emit(Statbot.EventType.MESSAGE, req.body);
    });
    ...
    pem.createCertificate({ selfSigned: true, days: 365 }, function(err, data) {
      ...
      statbot.promisedServerGetter_.resolve(statbot.enableSSL_ ?
          https.createServer(options, expressServer) : expressServer);
    });
  }
