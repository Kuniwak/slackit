...
BotFactory.buildReceivableOptions_ = function(opts, configObj) {
  ...
  opts.port = Number(receptOptions.port);
  assert(!isNaN(opts.port), 'Invalid port: ' + opts.port);

  if ('options' in receptOptions) {
    opts.outgoingHookURI = receptOptions.options.outgoingHookURI;
    opts.http = receptOptions.options.http;
  }
